...
def test_add_role_to_user_negative(
        flask_app,
        headers: dict,
        admin_token: str,
        created_user: User
    ...
    Добавление существующей у пользователя роли
    """
    ...
    assert resp.json == {'message': [f'The role {role.id} is already assigned to the user {created_user.id}.']}

def test_add_role_to_user_negative2(
        flask_app,
        headers: dict,
        admin_token: str,
        created_user: User
    ...
    assert resp.status_code == HTTPStatus.NOT_FOUND, "wrong status code"
    assert resp.json == {'message': ['Role is missing in db.']}
...
def test_add_role_to_user(
    ...
    """
    endpoint POST /auth_api/v1/user_roles/ positive test
    """
    ...
    assert resp.json['message'] == f'The {role.id} role has been added to the user {created_user.id}'
    assert role in created_user.roles
